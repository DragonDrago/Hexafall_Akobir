...
public class UserInterfaceManager : SuperClass
{
    ...
    public void StartGameButton()
    {
        ...
        //
        List<int> tempIndex = GetRandomIndex(7 * 3);

        for (int i = 0; i < (int)colorCountSlider.value; i++)
        {
            Color c = new Color(tempIndex[i] / 255f, tempIndex[i + 1] / 255f, tempIndex[i+2] / 255f, 1f);
            colors.Add(c);
        }
    ...
    public List<int> GetRandomIndex(int count, int first = 0, int last = 255)
    {
        List<int> indexs = new List<int>();
        int random = UnityEngine.Random.Range(first, last);

        indexs.Add(random);

        while (indexs.Count < count)
        {
            random = UnityEngine.Random.Range(first, last);
